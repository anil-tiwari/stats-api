package com.dunnhumby.stats_api.auth;
...
public class JwtTokenProvider {
    ...
    // Retrival of client id from the JWt token
    public String getClientId(String token) throws Exception {
        if(validateToken(token)){
            Jws<Claims> claims = Jwts.parser().verifyWith(this.secretKey)
                    .build().parseSignedClaims(token);

            return claims.getBody().get("clientId", String.class);
        }
        return null;
    }
